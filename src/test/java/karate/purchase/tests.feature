Feature: Simple Purchases
  ...
  Scenario: Purchase a single item
    ...
    And request { name: "Bananas", unitType: "WEIGHT", quantity: 140.0, pricePerUnit: 1.3, producer: "Farmer Joe", supplier: "Cargo bike dude", stockStatus: 'INSTOCK' }
    When method POST
    Then status 201
  ...
  Scenario: Purchase multiple items
    ...
    Given path '/stock'
    And header Authorization = "Bearer " + oToken
    And request { name: "Pumpkin", unitType: "PIECE", quantity: 20.0, pricePerUnit: 4.3, producer: "Farmer Joe", supplier: "Cargo bike dude", stockStatus: 'INSTOCK' }
    When method POST
    Then status 201
  ...
  Scenario: The price of a purchase does not change after a stock items price was updated
    ...
    Given path '/stock'
    And header Authorization = "Bearer " + oToken
    And request { name: "Bananas", unitType: "WEIGHT", quantity: 140.0, pricePerUnit: 1.0, producer: "Farmer Joe", supplier: "Cargo bike dude", stockStatus: 'INSTOCK' }
    When method POST
    Then status 201
    And def stockId1 = response.id
